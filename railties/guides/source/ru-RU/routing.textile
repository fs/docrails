...
Это руководство рассказывает о особенностях Rails роутинга. Ознакомившись с ним вы сможете:

* Понимать назначение роутинга
* Понимать код в +routes.rb+
* Создавать свои собственные роуты используя классический стиль с хэшэм или новый с использованием ресурсов
* Определять, как роуты будут соответствовать контроллеру и экшену

endprologue.

h3. Двойное назначение роутинга

Rails routing is a two-way piece of machinery - rather as if you could turn trees into paper, and then turn paper back into trees. Specifically, it both connects incoming HTTP requests to the code in your application's controllers, and helps you generate URLs without having to hard-code them as strings.

h4. Связываем URL с кодом

Кода Ваше приложение получает входящий HTTP запрос, скажем

<pre>
GET /patients/17
</pre>

то роутинг это та часть кода которая связывает запрос с определенным местом в вашем приложении. В данном случае, скорее всего будет вызван метод +show+ контроллера +patients+, отображающий информацию пациента с ID = 17.

h4. Создание URLa из кода

Рутинг работает и в обратную сторону. Если ваше приложение содержит код:
...
Тогда роутер переведет это в ссылку c URL +http://example.com/patients/17+. Используя роутинг таким способом, Вы можете сделать ваше приложение более надежным, по сравнению с приложением использующим жестко заданные URLы в коде. Так же код будет легче прочесть и понять.

NOTE: Для того, чтоб преобразование в URL через именованный роут работало, Patient должен быть объявлен как ресурс.

h3. Краткий обзор +routes.rb+
...
Роутинг состоит из 2 частей: сам роутер, который является частью Rails, и файл +config/routes.rb+, содержащий роуты которые будут использованы в вашем приложении. Изучение того, что Вы можете написать в +routes.rb+ является основной темой этого руководства, но прежде чем углубляться давайте сделаем краткий обзор.

h4. Обработка файла (Processing the File)

По сути, +routes.rb+ это один большой блок кода передаваемый в +ActionController::Routing::Routes.draw+. В этом блоке вы можете писать комментарии, но вероятно, большая его часть будет состоять из отдельных строк(, где каждая строка - путь в ваше приложение). Вы найдете пять основных типов роутов в этом файле:

* Ресурсы (RESTful Routes)
* Именованные роуты (Named Routes)
* Вложенные ресурсы (Nested Routes)
* Обычные роуты (Regular Routes)
* Роуты по умолчанию (Default Routes)

Позже мы рассмотрим подробнее каждый перечисленных типов роутов.

Когда призодит запрос, файл +routes.rb+ обрабатывается сверху вниз. Запрос обрабатывается первым подходящим роутом. Если пожходящего роута не находится, то Rails возврашает ошибку 404.

h4. Ресурсы (REST-роуты)

Ресурсы позволяют использовать преимущества REST принципов реализованных в Rails и всю информацию о роутинге записать всего одной строкой:

<ruby>
map.resources :books
</ruby>

h4. Именованные роуты

С помощью именованных роутов можно получить легко читаемые ссылки. Например:
...
Вложенные ресурсы позволяют объявить, что один ресурс содержится в другом. Похже Вы увидите как это объявление транслируется в URLы и пути к вашему коду. Например, если в вашем приложении есть parts, каждая из которых относится к assembly, тогда вы возможно создадите такой вложенные ресурс:
...
Во многих приложениях встречаются не REST-роуты, которые напрямую связывают части URL c конкретными экшенами. Например:

<ruby>
map.connect 'parts/:number', :controller => 'inventory', :action => 'show'
</ruby>

h4. Роуты по умолчанию

Для отлова запросов которым не соответствует ни один из объявленных роутов используют роуты по умолчанию. Многие приложения содержат такую пару роутов:
...
Эти роуты создаются автоматически при создании нового Rails приложения. Если вы используете только ресурсы вы можете удалить эти роуты, только убедитесь, что они не используются.

h3. RESTful Routing: the Rails Default

h3. REST роутинг: стандарт Rails

REST роутинг - текущий стандарт для роутинга в Rails, и вам следует использовать его для новых приложений. Потребуется некоторое время, для того, чтоб понять как работает REST роутинг, но это того стоит: ваш код будет легко читаться, вы будете работать с Rails а не бороться с ним (... when you use this style of routing)
...
В Rails, REST route обеспечивает связь между HTTP запросами, методами контроллера и операциями с базой данных. Одна запись в файле роутинга, например:

<ruby>
map.resources :photos
</ruby>

создает семь различных роутов в вашем приложении:

|_.HTTP запрос|_.URL           |_.контроллер|_.метод|_.используется для|
|GET          |/photos         |Photos      |index  |отображает список всех фотографий|
|GET          |/photos/new     |Photos      |new    |отображает HTML форму для создания новой фотографии|
|POST         |/photos         |Photos      |create |создает новую фотографию|
|GET          |/photos/1       |Photos      |show   |отображает указанную фотографию|
|GET          |/photos/1/edit  |Photos      |edit   |отображает HTML форму для редактирования фотографии|
|PUT          |/photos/1       |Photos      |update |обновляет указанную фотографию|
|DELETE       |/photos/1       |Photos      |destroy|удаляет указанную фотографию|

Для отдельных роутов (тех что ссылаются на единичный ресурс), в соответствующем методе контроллера, идентификатор ресурса будет доступен через +params[:id]+.

TIP: Если Вы всегда используете REST роуты в приложении, Вам следует отключить роуты по умолчанию заданные в +routes.rb+.

h4. URLы и пути

Creating a RESTful route will also make available a pile of helpers within your application:

Создание REST роута так же делает доступными следующие хелперы:

* +photos_url+ и +photos_path+ соответствуют URL'у для методов index и create 
* +new_photo_url+ и +new_photo_path+ соответствуют URL'у для метода new
* +edit_photo_url+ и +edit_photo_path+ соответствуют URL'у для метода edit
* +photo_url+ и +photo_path+ соответствуют URL'у для методов show, update и destroy
...
В каждом случае, +_url+ хелпер создает строку содержащую полный URL, а +_path+ хелпер создает строку содержащую относительный путь от корня приложения. Например:
...
h4. Одновременное определение нескольких ресурсов

Если вам нужно создать роуты для нескольких ресурсов, вы можете определить их все в одной строке:

<ruby>
map.resources :photos, :books, :videos
</ruby>

Это даст то же еффект, что и такая запись:
...
Вы так же можете создать роуты для единичного ресурса. Для этого нужно использовать +map.resource+ вместо +map.resources+, тогда будут созданы другие роуты. Например такая запись:

<ruby>
map.resource :geocoder
</ruby>

создаст шесть различных роутов для вашего приложения:

|_.HTTP verb|_.URL           |_.контроллер|_.метод  |_.используется для|
