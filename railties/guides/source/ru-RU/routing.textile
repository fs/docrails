...
роутинг это та часть кода которая связывает запрос с определенным местом в вашем приложения. В данном случае, приложение скорее всего запустит метод +show+ контроллера +patients+, отображающий информацию пациента с ID 17.
...
Тогда роутер переведет это в ссылку c URL +http://example.com/patients/17+. Используя роутинг таким способом, Вы можете сделать ваше приложении более надежным, в сравнении с приложением использующим жестко заданные URLы в коде, так же код будет легче прочесть и понять.

NOTE: Patient должен быть объявлен как ресурс для того, чтоб преобразование в URL через именованный роут было доступно.
