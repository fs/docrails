...
-- _Перенаправление в веб-приложениях - это недооцененный инструмент взломщика: Не только атакующий может направить пользователя на сайт-ловушку, он может создать автономную атаку._

Всякий раз, когда клиенту посылается адрес (либо его часть) для перенаправления, это создает потенциальную уязвимость. Наиболее очевидной атакой будет перенаправить пользователя на поддельный сайт, который будет выглядеть как оригинальный сайт. Это так называемая атака-фишинг. Она работает посредством посылки не вызывающей подозрения ссылки пользователю по email, инъекцией через XSS в веб-приложении или помещением ссылки на другой сайт. Ссылка не вызывает подозрений, поскольку начинается настоящим адресом веб-приложения, а адрес подложного сайта скрыт в перенаправляющем параметре: http://www.example.com/site/redirect?to= www.attacker.com. Here is an example of a legacy action:
...
Это перенаправит пользователя на действие main всякий раз, когда он попытается обратиться к действию legacy. Смысл этого в том, чтобы сохранить параметры переданные действию legacy и передать их действию main. Однако, это может использовано атакующим в качестве уязвимости, если он включит подложный URL в качестве параметра:

<plain>
http://www.example.com/site/legacy?param1=xy&param2=23&host=www.attacker.com
</plain>

Поскольку подложный адрес передается в качестве последнего параметра, довольно сложно заподозрить атаку обычному пользователю. Простой мерой предотвращения данной атаки будет _(highlight)проверять перенаправляющий параметр перед тем как отправить пользователя по указанному в нём адресу_.

h5. Автономный XSS

Другой тип атак перенаправлением работает в Firefox и Opera через использование data-протокола. Этот протокол отображает содержимое прямо в браузере и может быть все, чем угодно, от HTML или JavaScript до изображений:

+data:text/html;base64,PHNjcmlwdD5hbGVydCgnWFNTJyk8L3NjcmlwdD4K+

Этот пример содержит Base64 закодированные JavaScript отображающий простое сообщение в диалоговом окне. В качестве адреса перенаправления атакующий может передать такой адрес, содержащий вредоносный код. В качестве контрмеры, _(highlight)нельзя позволять пользователю изменять адрес перенаправления (или его часть)_.
